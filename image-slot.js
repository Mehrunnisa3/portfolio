...
(() => {
  ...
  function save() {
    ...
    if (!w) return;
    saving = true;
    // Re-read the sidecar right before writing and merge it under the
    // in-memory copy. If a page reload reset this module mid-flight (e.g. a
    // design edit landed right after a drop), `slots` here could be a fresh,
    // near-empty object that only knows about the just-dropped id — writing
    // it verbatim would blank out every other slot on disk. Merging keeps
    // every id we don't currently know about instead of clobbering it.
    fetch(STATE_FILE, { cache: 'no-store' })
      .then((r) => (r.ok ? r.json() : null))
      .then((disk) => {
        const base = disk && typeof disk === 'object' ? disk : {};
        ...
        for (const id of tombstones) delete merged[id];
        return w(STATE_FILE, JSON.stringify(merged));
      })
      .catch(() => {})
  ...
  class ImageSlot extends HTMLElement {
    ...
    _commitView() {
      ...
      if (this._userUrl) v.u = this._userUrl;
        ...
        // Reframing an image loaded from the sidecar (not dropped this
        // session) — keep its existing u so this commit doesn't blank it.
        const existing = this.id ? getSlot(this.id) : this._local;
        if (existing && existing.u) v.u = existing.u;
      }
